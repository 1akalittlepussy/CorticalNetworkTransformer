...
def diff_multi_head_attention_forward(query,
    ...
    #mean = attn_output_weights.mean(dim=-1)
    #mean_final = mean1.long().sum(dim=-1)
    #attn = attn_output_weights.long().squeeze(1)
    #attn_output_numpy = mean.cpu().detach().numpy()
    #scipy.io.savemat('attnHCP.mat', {'data': attn_output_numpy})

    attn_output = torch.bmm(attn_output_weights, v)
    #np.savetxt('attn.txt', attn_output_numpy)  # 将数组a的数据保存入文件a.txt中
    #attn_output_ = attn_output
    #attn_output_.detach().numpy()
    #np.savetxt('./attn.csv', attn_output_, fmt='%.2f', delimiter=',')
    #print(attn_output)
...
class DAFF(nn.Module):
    def __init__(self, in_features, hidden_features=62, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        ...
        #self.squeeze = global_add_pool(x, batch)
...
class DiffTransformerEncoderLayer(nn.TransformerEncoderLayer):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.scaling = None
        self.daff = DAFF(in_features=d_model, hidden_features=d_model, act_layer=nn.GELU, drop=0.1)
        self.linear1 = nn.Linear(in_features=d_model,out_features=62,bias=False)

    def forward(self, src, pe,adj, degree=None, src_mask=None, src_key_padding_mask=None):
