...
class GraphTransformer(nn.Module):
    def __init__(self, in_size, num_class, d_model, num_heads=8,
                 ...
                 gnn_type="graph", se="khopgnn", use_edge_attr=False, num_edge_features=4,
    ...
    def forward(self, data, return_attn=False):
        ...
        x = torch.from_numpy(scaler.fit_transform(x.cpu()))
        ...
        output = self.embedding(x) if node_depth is None else self.embedding(x.long(), node_depth.view(-1,))#RuntimeError: Expected tensor for argument #1 'indices' to have scalar type Long; but got torch.cuda.FloatTensor instead
